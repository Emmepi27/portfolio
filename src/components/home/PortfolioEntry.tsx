...
import { cinemaProjects, type Project } from "@/content/projects";
import { usePortfolioHomeMotion } from "./usePortfolioHomeMotion";

const DEFAULT_FEATURED = ["rsfly", "jiwa-creative-studio"] as const;
...
/** Due film in evidenza + link descrittivi al cinema e ai case study. */
export default function PortfolioEntry({ featuredSlugs = DEFAULT_FEATURED }: PortfolioEntryProps) {
  const sectionRef = useRef<HTMLElement>(null);
  const featured = featuredSlugs
    .map((slug) => cinemaProjects.find((p) => p.slug === slug))
    ...
    >
      <div className="mx-auto max-w-6xl px-5">
        <div data-portfolio-intro>
          ...
          >
            Cinema: vincoli, stack, risultati
          </h2>
          <p className="mt-3 max-w-2xl text-sm leading-[1.65] text-[color:var(--ds-text-secondary)] sm:mt-4 sm:text-base sm:leading-[1.6]">
            Ogni film e un case study con problema, vincoli e impatto: non solo screenshot. Nel{" "}
            <Link href="/work" className="ds-link-accent font-medium">
              cinema
            </Link>{" "}
            trovi la selezione completa e un film in evidenza; qui sotto due ingressi diretti ai dettagli.
          </p>
        </div>
            ...
            return (
              ...
              >
                <Link
                  ...
                  aria-label={`Anteprima: ${p.title} - apri film`}
                >
                  {thumb ? (
                ...
                <div
                  className="flex min-w-0 flex-1 flex-col lg:py-1"
                  data-portfolio-secondary-copy={!isLead ? "true" : undefined}
                >
                  <div data-portfolio-meta>
                    <p className="text-xs font-medium text-[color:var(--ds-text-secondary)]">
                      {isLead ? "In primo piano" : "Stesso metodo, altro contesto"}
                    </p>
                  ...
                  <h3
                    data-portfolio-title={isLead ? "true" : undefined}
                    className={cn(
                      "mt-4 font-[var(--font-serif)] font-bold tracking-tight text-[color:var(--ds-text-primary)]",
                      isLead
                        ? "text-2xl leading-[1.15] sm:text-3xl sm:leading-tight"
